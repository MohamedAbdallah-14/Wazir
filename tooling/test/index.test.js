...
function createTempStateRoot() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'wazir-storage-'));
...
function createIndexFixture() {
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wazir-index-'));
...
describe('wazir index and recall commands', () => {
