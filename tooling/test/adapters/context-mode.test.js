...
describe('isEnabled', () => {
  ...
  test('returns false when WAZIR_DISABLE_CONTEXT_MODE is set', () => {
    const prev = process.env.WAZIR_DISABLE_CONTEXT_MODE;
    try {
      process.env.WAZIR_DISABLE_CONTEXT_MODE = '1';
      assert.strictEqual(isEnabled({ context_mode: { enabled: true } }), false);
    } finally {
      if (prev === undefined) {
        delete process.env.WAZIR_DISABLE_CONTEXT_MODE;
      } else {
        process.env.WAZIR_DISABLE_CONTEXT_MODE = prev;
      }
    }
  });
});
