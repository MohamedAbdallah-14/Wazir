...
describe('CI workflow — validation steps', () => {
  ...
  test('has "Require changelog entries" step for feat/feature/fix/codex/hotfix PRs', () => {
    const ci = loadCI();
    const steps = ci.jobs.verify.steps;
    ...
    assert.match(step.run, /--base/, 'must use --base flag');
    assert.match(step.run, /--head/, 'must use --head flag');
    // Check branch prefix conditions — each must appear as a startsWith check
    const ifCond = step.if;
    for (const prefix of ['feat/', 'feature/', 'fix/', 'codex/', 'hotfix/']) {
      assert.match(
        ifCond,
        new RegExp(`startsWith\\(github\\.head_ref, '${prefix.replace('/', '\\/')}'\\)`),
        `if condition must check startsWith for ${prefix} prefix`,
      );
    }
  });
});
