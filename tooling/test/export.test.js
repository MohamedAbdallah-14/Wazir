...
function createExportFixture() {
  const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'wazir-export-'));
  ...
  fs.copyFileSync(
    path.join(ROOT, 'schemas', 'wazir-manifest.schema.json'),
    path.join(fixtureRoot, 'schemas', 'wazir-manifest.schema.json'),
  );
  fs.copyFileSync(
...
describe('wazir export command', () => {
  ...
  test('builds host export packages and manifests for every supported host', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(fixture.fixtureRoot, 'exports', 'hosts', 'cursor', '.cursor', 'rules', 'wazir-core.mdc')));
  ...
  test('cleans up stale host scratch directories before rebuilding exports', () => {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(staleBackupDir, '.wazir-export-scratch.json'),
        JSON.stringify({ kind: 'backup', host: 'claude' }, null, 2),
      );
      fs.writeFileSync(
        path.join(staleStagedDir, '.wazir-export-scratch.json'),
...
describe('claude export agent frontmatter and CLAUDE.md content', () => {
