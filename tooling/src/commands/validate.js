...
export function validateManifestAtProjectRoot(projectRoot) {
  ...
  const schemaPath = path.join(projectRoot, 'schemas', 'wazir-manifest.schema.json');
...
export function runValidateCommand(parsed, context = {}) {
  ...
  switch (parsed.subcommand) {
    ...
    case 'artifacts':
      return notImplemented('wazir validate artifacts');
    case 'docs':
    ...
    case 'branches':
      return validateBranchName(extractFlag(parsed.args, '--branch'), { cwd: projectRoot });
    case 'commits':
    ...
    default: {
      ...
      if (parsed.args.length > 0) {
        return failure('Flags require a subcommand. Specify a subcommand, e.g.: wazir validate commits --base HEAD~5');
      }
